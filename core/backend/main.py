...
# Add Project Root and local paths for basicsr and Real-ESRGAN
project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
core_path = os.path.join(project_root, "core")
real_esrgan_path = os.path.join(project_root, "Real-ESRGAN")
...
try:
    ...
    weights_dir = os.path.join(real_esrgan_path, "weights")
    esrgan_weights = os.path.join(weights_dir, "RealESRGAN_x4plus.pth")
    
    # Auto-download weights if missing
    if not os.path.exists(esrgan_weights):
        print(f"Real-ESRGAN weights not found. Downloading to {esrgan_weights}...")
        import requests
        os.makedirs(weights_dir, exist_ok=True)
        url = "https://github.com/xinntao/Real-ESRGAN/releases/download/v0.1.0/RealESRGAN_x4plus.pth"
        r = requests.get(url, allow_redirects=True)
        with open(esrgan_weights, 'wb') as f:
            f.write(r.content)
